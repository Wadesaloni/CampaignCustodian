📧 Intelligent Email QA Agent for Hackathon
Analyzes email HTML, compares with reference images, validates against brand guidelines,
and provides enhanced QA metrics: readability, spam risk, link validation, and design consistency.
...
# ======================================================================
class LLMClient:
    """OpenAI client for GPT-4o usage"""
...
class CopyQAAgent:
    ...
    def analyze_copy_quality(self, text_content: Dict, brand_guidelines: str) -> Dict:
        prompt = f"""You are an expert email QA analyst. Analyze this email against brand guidelines.
...
"""
    ...
    def compare_with_reference_image(self, html_content: str, reference_image_bytes: bytes, text_content: Dict) -> Dict:
        ...
        prompt = f"""Compare reference email design with actual content:
...
Analyze visual consistency, element placement, content accuracy, missing elements.
Return JSON: {{visual_match_score: 0-100, discrepancies: [], missing_elements: [], layout_consistency: 0}}
"""
        messages = [
    ...
    def analyze_readability(self, text_content: Dict) -> Dict:
        return {
            "flesch_reading_ease": textstat.flesch_reading_ease(text_content['body_text']),
            "smog_index": textstat.smog_index(text_content['body_text']),
    ...
    def analyze_spam_risk(self, text_content: Dict) -> Dict:
        spam_words = ["free","buy now","urgent","act now","guarantee"]
        found = [w for w in spam_words if w.lower() in text_content['body_text'].lower()]
    ...
    def validate_links(self, text_content: Dict) -> Dict:
        ...
        for link in text_content['links']:
            if link in seen: duplicates.append(link)
            else: seen.add(link)
            if re.match(url_pattern, link):
                valid.append(link)
            else:
                invalid.append(link)
        return {"valid_links": valid, "invalid_links": invalid, "duplicate_links": duplicates}
    ...
    def generate_report(self, html_content: str, reference_image_bytes: Optional[bytes], brand_guidelines: str) -> QAReport:
        ...
        if readability["flesch_reading_ease"]<50:
            recommendations.append("Content hard to read. Simplify sentences.")
        if spam_risk["spam_risk_score"]>40:
            recommendations.append("High spam risk detected.")
        if link_validation["invalid_links"]:
            recommendations.append(f"Invalid links found: {link_validation['invalid_links']}")

        visual_analysis.update({"readability": readability, "spam_risk": spam_risk, "link_validation": link_validation})
...
def main():
    ...
    st.write("Analyze email HTML against brand guidelines with enhanced QA metrics.")
    ...
    if st.sidebar.button("Run QA Analysis"):
        if not html_file: st.sidebar.error("Please upload an HTML file."); st.stop()
        ...
        if checklist_file:
            try: df = pd.read_excel(checklist_file); brand_guidelines = df.to_string(index=False)
            except: brand_guidelines="Error reading checklist."
        else: brand_guidelines="No checklist provided."
            ...
            if report.grammar_issues:
                for issue in report.grammar_issues:
                    with st.expander(f"{issue['location']} | {issue['severity'].capitalize()}"):
                        st.write(issue['issue']); st.markdown(f"**Suggestion:** {issue['suggestion']}")
            else: st.success("No grammar issues detected ✅")
        ...
        with tabs[2]:
            ...
            else: st.success("No content deviations detected ✅")
