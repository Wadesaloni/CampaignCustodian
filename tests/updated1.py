...
{{
    ...
    def analyze_accessibility(self, html_content: str) -> Dict:
        soup = BeautifulSoup(html_content, 'html.parser')
        images_without_alt = [img['src'] for img in soup.find_all('img') if not img.get('alt')]
        links_with_generic_text = [a.get_text().strip() for a in soup.find_all('a')
                                   if a.get_text().strip().lower() in ['click here', 'read more']]
        recommendation = "Add alt text to images and use descriptive link text." if images_without_alt or links_with_generic_text else ""
        return {
            "images_missing_alt": images_without_alt,
            "generic_link_text": links_with_generic_text,
            "score": max(0, 100 - (len(images_without_alt)*10 + len(links_with_generic_text)*5)),
            "recommendation": recommendation
        }

    def analyze_spam_risk(self, text_content: Dict) -> Dict:
        spam_keywords = ["free", "buy now", "limited offer", "click here"]
        subject = text_content['subject'].lower()
        risk_flags = [kw for kw in spam_keywords if kw in subject]
        caps_ratio = sum(1 for c in text_content['subject'] if c.isupper()) / max(len(text_content['subject']),1) * 100
        return {
            "spam_keywords_detected": risk_flags,
            "caps_ratio": caps_ratio,
            "risk_score": min(100, len(risk_flags)*20 + caps_ratio)
        }

    def analyze_cta_effectiveness(self, text_content: Dict) -> Dict:
        action_verbs = ["buy", "shop", "download", "register", "get"]
        weak_ctas = [cta for cta in text_content['cta_buttons'] if not any(v in cta.lower() for v in action_verbs)]
        return {
            "total_ctas": len(text_content['cta_buttons']),
            "weak_ctas": weak_ctas,
            "score": max(0, 100 - len(weak_ctas)*15)
        }

    def analyze_readability(self, text_content: Dict) -> Dict:
        body_text = text_content['body_text']
        score = textstat.flesch_reading_ease(body_text)
        long_sentences = [s for s in body_text.split('.') if len(s.split()) > 30]
        return {
            "flesch_score": score,
            "long_sentences_count": len(long_sentences),
            "recommendation": "Consider shortening long sentences for better engagement." if long_sentences else ""
        }

    # Update generate_report to include new analyses
    def generate_report(self, html_content: str, reference_image_bytes: Optional[bytes], brand_guidelines: str) -> QAReport:
        text_content = self.extract_text_from_html(html_content)
        copy_analysis = self.analyze_copy_quality(text_content, brand_guidelines)
        visual_analysis = self.compare_with_reference_image(html_content, reference_image_bytes, text_content) if reference_image_bytes else {"visual_match_score":0,"discrepancies":[],"missing_elements":[],"layout_consistency":0}

        # New bonus analyses
        accessibility = self.analyze_accessibility(html_content)
        spam_risk = self.analyze_spam_risk(text_content)
        cta_analysis = self.analyze_cta_effectiveness(text_content)
        readability = self.analyze_readability(text_content)

        # Calculate overall score
        tone_score = copy_analysis.get('tone_analysis', {}).get('consistency', 0)
        visual_score = visual_analysis.get('visual_match_score', 0)
        bonus_score = (accessibility["score"] + 100 - spam_risk["risk_score"] + cta_analysis["score"] + readability["flesch_score"]) / 4
        overall_score = (tone_score * 0.4 + visual_score * 0.3 + bonus_score * 0.3)

        # Compile recommendations
        recommendations = copy_analysis.get('recommendations', [])
        if accessibility.get('recommendation'):
            recommendations.append(accessibility['recommendation'])
        if spam_risk['spam_keywords_detected']:
            recommendations.append(f"Spam risk detected: {spam_risk['spam_keywords_detected']}")
        if cta_analysis['weak_ctas']:
            recommendations.append(f"Weak CTAs: {cta_analysis['weak_ctas']}")
        if readability.get('recommendation'):
            recommendations.append(readability['recommendation'])
...
def main():
    ...
    checklist_file = st.file_uploader("📋 Upload Brand Checklist / Guidelines", type=["txt", "md", "xlsx"])
